...
// ... (interface Card e constante materials permanecem as mesmas) ...
interface Card {
  id: number;
  material: string;
  emoji: string;
      ...
      <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6 mb-6">
        <div className="flex flex-col sm:flex-row justify-between sm:items-center mb-6 gap-4 sm:gap-0">
          <div className="flex flex-wrap gap-x-4 gap-y-2 sm:gap-x-6">
            <div className="flex items-center gap-2">
              ...
              <span className="font-semibold text-sm sm:text-base">Pontos: {calculateScore()}</span>
            </div>
            <div className="flex items-center gap-2">
              <Timer className="text-blue-600" size={20} />
              <span className="font-semibold text-sm sm:text-base">{formatTime(timeElapsed)}</span>
            </div>
            <div className="flex items-center gap-2">
              <span className="font-semibold text-sm sm:text-base">Movimentos: {moves}</span>
            </div>
            <div className="flex items-center gap-2">
              <span className="font-semibold text-sm sm:text-base">Pares: {matches}/{materials.length}</span>
            </div>
          </div>
          <button
        ...
        {gameOver ? (
          <div className="text-center py-8">
            ...
            <h3 className="text-xl sm:text-2xl font-bold mb-2">Parabéns!</h3>
            ...
            <div className="grid grid-cols-4 gap-2 sm:gap-4">
              {cards.map((card, idx) => (
                <button
                  ...
                  className={`aspect-square text-4xl sm:text-5xl rounded-lg transition-all duration-300 transform ${
            ...
            {leaderboard.map((entry, idx) => (
              <div key={entry.id} className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                ...
                <div className="text-right">
                  <div className="font-bold text-blue-600 text-sm sm:text-base">{entry.score} pts</div>
                  {entry.time_taken && (
                    <div className="text-xs sm:text-sm text-gray-500">{formatTime(entry.time_taken)}</div>
                  )}
                </div>
              </div>
            ))}
